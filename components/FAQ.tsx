      ...
      <div className="container mx-auto px-4 max-w-4xl">
        <div className="text-center mb-20 space-y-4">
          ...
          <p className="text-xl text-slate-600 dark:text-slate-400 font-medium">
            {isRtl ? 'كل اللي محتاج تعرفه عن خدمات HomeServe Pro' : 'Everything you need to know about our services'}
          </p>
        </div>
        ...
        <div className="space-y-6">
          {faqs.map((faq, idx) => (
            <div 
              key={idx} 
              className={`bg-white dark:bg-slate-900 rounded-[2rem] border transition-all duration-300 ${
                openIdx === idx ? 'border-primary shadow-2xl ring-4 ring-primary/5' : 'border-slate-100 dark:border-slate-800'
              } overflow-hidden`}
            >
              <button 
                onClick={() => setOpenIdx(openIdx === idx ? null : idx)}
                className="w-full px-10 py-8 flex items-center justify-between text-start gap-6 group"
              >
                <span className={`text-xl md:text-2xl font-black transition-colors ${openIdx === idx ? 'text-primary' : 'text-slate-900 dark:text-white group-hover:text-primary'}`}>
                  {isRtl ? faq.qAr : faq.qEn}
                </span>
                <div className={`p-2 rounded-xl transition-all ${openIdx === idx ? 'bg-primary text-white rotate-180' : 'bg-slate-50 dark:bg-slate-800 text-slate-400 group-hover:bg-primary group-hover:text-white'}`}>
                  <ChevronDown size={24} strokeWidth={3} />
                </div>
              </button>
              {openIdx === idx && (
                <div className="px-10 pb-10 text-xl text-slate-600 dark:text-slate-300 font-bold leading-relaxed animate-in fade-in slide-in-from-top-4 duration-300">
                  <div className={`w-12 h-1.5 bg-primary/20 rounded-full mb-6 ${isRtl ? 'ml-auto' : 'mr-auto'}`}></div>
